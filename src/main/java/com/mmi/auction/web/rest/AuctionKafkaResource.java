...
@RestController
@RequestMapping("/api/auction-kafka")
public class AuctionKafkaResource {

    private final Logger log = LoggerFactory.getLogger(AuctionKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public AuctionKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
